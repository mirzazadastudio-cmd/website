...
import {animationClips} from '@/lib/animation-clips';
function MotionClip({clip}:{clip:(typeof animationClips)[number]}){
...
export function AnimationSection({compact=false}:{compact?:boolean}){
 ...
 <div className="section-title"><div><p className="eyebrow">THE STUDIO / IN MOTION</p>{compact?<h2>Five seconds. A new perspective.</h2>:<h1>Architecture in motion.</h1>}</div><div><p>Short films of spaces, systems and changing light.</p>{compact&&<a className="text-link" href="/animation">Explore animation <ArrowUpRight size={19}/></a>}</div></div>
 <div className="motion-grid">{animationClips.map(clip=><MotionClip key={clip.slug} clip={clip}/>)}</div>
 </section>;
}

...
export function CollectionView({slug}:{slug:string}){
 ...
 return <><section className="section collection-intro"><a className="back-link" href="/#projects">← All projects</a><div className="collection-heading"><div><p className="eyebrow">THE COLLECTION / {String(items.length).padStart(2,'0')} PROJECTS</p><h1>{collection.category}</h1></div><div><p>{collection.description}</p><span>{collection.scope}</span></div></div><nav className="collection-nav" aria-label="Project collections">{collections.filter(c=>c.category==='AI'||collectionProjects(projects,c).length>0).map(c=><a key={c.slug} href={'/collections/'+c.slug} aria-current={c.slug===slug?'page':undefined}>{c.category}</a>)}</nav></section>

...
export function setSnapshot(snapshot:typeof record){Object.assign(record,{...snapshot,data:publicContent(upgradeCatalog(snapshot.data))});routes.splice(0,routes.length,...["/","/animation","/about","/services","/projects","/blog","/contact","/admin",...record.data.projects.map(p=>"/projects/"+p.slug),...record.data.collections.map(c=>"/collections/"+c.slug),...record.data.journal.map(p=>"/blog/"+p.slug)]);}
